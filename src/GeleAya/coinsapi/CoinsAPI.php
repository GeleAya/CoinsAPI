...
use GeleAya\coinsapi\provider\Provider;
use GeleAya\coinsapi\provider\YamlProvider;
use GeleAya\coinsapi\provider\MySQLProvider;
use GeleAya\coinsapi\event\money\SetMoneyEvent;
use GeleAya\coinsapi\event\money\ReduceMoneyEvent;
use GeleAya\coinsapi\event\money\AddMoneyEvent;
use GeleAya\coinsapi\event\money\MoneyChangedEvent;
use GeleAya\coinsapi\event\account\CreateAccountEvent;
use GeleAya\coinsapi\task\SaveTask;
...
class CoinsAPI extends PluginBase implements Listener{
	...
	private function registerCommands(){
		...
		$commands = [
			"mymoney" => "\\GeleAya\\coinsapi\\command\\MyMoneyCommand",
			"topmoney" => "\\GeleAya\\coinsapi\\command\\TopMoneyCommand",
			"setmoney" => "\\GeleAya\\coinsapi\\command\\SetMoneyCommand",
			"seemoney" => "\\GeleAya\\coinsapi\\command\\SeeMoneyCommand",
			"givemoney" => "\\GeleAya\\coinsapi\\command\\GiveMoneyCommand",
			"takemoney" => "\\GeleAya\\coinsapi\\command\\TakeMoneyCommand",
			"pay" => "\\GeleAya\\coinsapi\\command\\PayCommand",
		];
		foreach($commands as $cmd => $class){
			$map->register("coinsapi", new $class($this));
		}
	}
